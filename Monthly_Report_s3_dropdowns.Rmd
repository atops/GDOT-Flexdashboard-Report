...
maint_health_data <- reactive({
    ...
    if (!is.null(input$zone_group) & !is.null(input$corridor_x) & !is.null(input$subcorridors)) {
        data_ <- if (corridor() == "All Corridors" | input$subcorridors == "By Sub-corridor") {
            sub$mo$maint
        } else {
            sig$mo$maint
        }
        
        get_health_data_filtered(data_, zone_group(), corridor()) %>%
            filter(Month == current_month()) %>%
            ungroup() %>%
...
observe({
    if (!is.null(input$zone_group) & !is.null(input$corridor_x)) {
        ...
        if (input$corridor_x == "All Corridors") {
            ...
            choices_ <- c("Percent Health", "Missing Data", "Detection Uptime Score",
                          "Ped Actuation Uptime Score", "Comm Uptime Score",
                          "CCTV Uptime Score", "RSU Uptime Score", "Flash Events Score",
                          "Detection Uptime", "Ped Actuation Uptime", "Comm Uptime",
                          "CCTV Uptime", "RSU Uptime")
        }
...
fillRow(
    plotlyOutput("health_maint_plot"), height = "85%"
)

output$health_maint_plot <- renderPlotly({
        get_bar_line_dashboard_plot(
            cor_weekly = rename(mr()$mo$maint_plot, Date = Month),  # maint$sub
            cor_monthly = mr()$mo$maint_plot, 
            cor_hourly = NULL, 
            var_ = input$maintenance_var,  #  "Percent Health", 
            num_format = "percent",
            highlight_color = GDOT_BLUE, 
            month_ = current_month(), 
            zone_group_ = zone_group(),
            x_bar_title = input$maintenance_var,
            x_line1_title = glue("{input$maintenance_var} trend"),
            plot_title = input$maintenance_var,
            accent_average = FALSE)
})
```
...
observe({
    if (!is.null(input$zone_group) & !is.null(input$corridor_x)) {
        ...
        if (input$corridor_x == "All Corridors") {
            ...
            choices_ <- c("Percent Health", "Missing Data", "Platoon Ratio Score", 
                        "Ped Delay Score", "Split Failures Score", 
                        "Travel Time Index Score", "Buffer Index Score", 
                        "Platoon Ratio", "Ped Delay", "Split Failures", 
...
fillRow(
    plotlyOutput("health_ops_plot"), height = "85%"
)

output$health_ops_plot <- renderPlotly({
    get_bar_line_dashboard_plot(
        cor_weekly = rename(mr()$mo$ops_plot, Date = Month), 
        cor_monthly = mr()$mo$ops_plot, 
        cor_hourly = NULL, 
        var_ = input$operations_var,  #  "Percent Health", 
        num_format = "percent",
        highlight_color = GDOT_BLUE, 
        month_ = current_month(), 
        zone_group_ = zone_group(),
        x_bar_title = input$operations_var,
        x_line1_title = glue("{input$operations_var} trend"),
        plot_title = input$operations_var,
        accent_average = FALSE)
})
```

### Safety
```{r Safety, warning = FALSE}
