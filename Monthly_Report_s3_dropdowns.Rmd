...
output$tp_valuebox1 <- output$tp_valuebox2 <- renderValueBox({
    valueBox(
        value = get_valuebox_value(
            throughput,
            zone_group = zone_group(), month = current_month(), line_break = TRUE
        ),
        icon = "fa-bar-chart",
        color = BLUE
    )
...
`r tags$a(href = "http://sigopsmetrics.com/editor",
          target = "_blank",
          "Click here to edit progress reports")`
...
observe({
    if (!is.null(input$zone_group) & !is.null(input$corridor_x)) {
        if (input$corridor_x == "All Corridors") {
            ...
            choices_ <- c(
                "Percent Health", "Missing Data", "Platoon Ratio Score",
