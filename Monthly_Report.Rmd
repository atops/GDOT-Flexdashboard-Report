...
renderPlot({
    if(corridor() == "All Corridors") {
        if (zone_group() == "All RTOP") {
            df <- filter(sig$dy$cctv, Zone_Group %in% c("RTOP1", "RTOP2"))
        } else {
            df <- filter(sig$dy$cctv, Zone_Group == zone_group())
        }
    } else {
        df <- filter(sig$dy$cctv, Corridor == corridor())
    }
    plot_cctvs(df, month_ = current_month())
})
```
