...
Past month reports can be viewed back to `r conf$report_start_date`.

```{r sidebar, warning = FALSE, cache = FALSE}
# shiny inputs defined here

if (conf$mode == "production") {
    current_month <- ymd(conf$production_report_end_date)   # Production

} else if (conf$mode == "beta") {
    current_month <- today() - days(6)   # Beta
    
} else {
    stop("mode defined in configuration yaml file must be either production or beta")
}    

first_month <- current_month - months(12)
report_months <- seq(current_month, first_month, by = "-1 month")
