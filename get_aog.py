...
def get_det_config(date_, conf):
    ...
    dc_key = (
        f"config/atspm_det_config_good/date={date_str}/ATSPM_Det_Config_Good.feather"
    )
    with io.BytesIO() as data:
        boto3.resource("s3").Bucket(bucket).download_fileobj(Key=dc_key, Fileobj=data)
    ...
    df = pd.merge(dc, bd, how="outer", on=["SignalID", "Detector"]).fillna(
        value={"Good_Day": 1}
    )
...
def get_aog(signalid, date_, det_config, conf, per="H"):
    ...
    try:
        ...
        all_hours = pd.date_range(
            date_, date_ + pd.Timedelta(1, unit="days"), freq=per, inclusive="left"
        )
        ...
        if df.empty:
            ...
            df_aog = (
                df.assign(Hour=lambda x: x.DetTimeStamp.dt.floor(per))
                .rename(columns={"Detector": "Arrivals", "EventCode": "Interval"})
                .groupby(["Hour", "SignalID", "Phase", "Interval"])
                .count()[["Arrivals"]]
            )
            df_aog["All_Arrivals"] = df_aog.groupby(
                level=["Hour", "SignalID", "Phase"]
            ).transform("sum")
            df_aog["AOG"] = df_aog["Arrivals"] / df_aog["All_Arrivals"]

            # Filter only the arrivals on Interval=1, Green
            aog = (
                df_aog.reset_index("Interval")
                .query("Interval == 1")
            ...
            df_gc = (
                df[["SignalID", "Phase", "PhaseStart", "EventCode"]]
                .drop_duplicates()
                .rename(
                    columns={"PhaseStart": "IntervalStart", "EventCode": "Interval"}
                )
            ...
            x = pd.DataFrame(
                ...
                index=pd.MultiIndex.from_product(
                    [df_gc.index.levels[0], df_gc.index.levels[1], all_hours],
            ...
            df_gc["IntervalEnd"] = df_gc.groupby(level=["SignalID", "Phase"]).shift(-1)[
                "IntervalStart"
            ]
            df_gc["IntervalDuration"] = (
                df_gc.IntervalEnd - df_gc.IntervalStart
            ).dt.total_seconds()
            df_gc["Hour"] = df_gc.IntervalStart.dt.floor(per)

            df_gc = df_gc.reset_index(drop=False).set_index(
                ["Hour", "SignalID", "Phase", "Interval"]
            )

            interval_duration = df_gc.groupby(
                level=["Hour", "SignalID", "Phase", "Interval"]
            )["IntervalDuration"].sum()
            phase_duration = interval_duration.groupby(
                level=["Hour", "SignalID", "Phase"]
            ).sum()
            gC = interval_duration / phase_duration

            gC = (
                gC.reset_index("Interval")
                .query("Interval == 1")
                .drop(columns=["Interval"])
                .rename(columns={"IntervalDuration": "gC"})
            )

            aog = pd.merge(aog, gC, left_index=True, right_index=True).assign(
                pr=lambda x: x.AOG / x.gC
            )
        ...
        try:
            ...
            with get_context("spawn").Pool(processes=24) as pool:
                results = pool.starmap_async(
                    ...
                    list(
                        itertools.product(
                            signalids, [date_], [det_config], [conf], ["H"]
                        )
                    ),
                )
                pool.close()
            ...
            df = (
                pd.concat(dfs)
                .reset_index()[
                    ["SignalID", "Phase", "Hour", "AOG", "pr", "All_Arrivals"]
                ]
                .rename(
                    columns={
                        "Phase": "CallPhase",
                ...
                .fillna(value={"vol": 0})
                .assign(
                    SignalID=lambda x: x.SignalID.astype("str"),
            ...
            df.to_parquet(
                f"s3://{bucket}/mark/arrivals_on_green/date={date_str}/aog_{date_str}.parquet"
            )
            ...
            with get_context("spawn").Pool(processes=24) as pool:
                results = pool.starmap_async(
                    ...
                    list(
                        itertools.product(
                            signalids, [date_], [det_config], [conf], ["15min"]
                        )
                    ),
                )
                pool.close()
            ...
            df.to_parquet(
                f"s3://{bucket}/mark/arrivals_on_green_15min/date={date_str}/aog_{date_str}.parquet"
            )
