...
if not os.path.exists(logs_path):
    os.mkdir(logs_path)
logger = mark1_logger(
    os.path.join(logs_path, f'get_cctv_{now("US/Eastern").strftime("%F")}.log')
)
...
def get_camids_s3(conf):
    camids = list(
        s3io.read_excel(
            Bucket=conf["bucket"], Key=conf["cctv_config_filename"]
        ).CameraID.values
    )
    return camids
...
def get_camids_nav():
    ...
    try:
        ...
        camids = sorted(
            list(set(re.findall("(?<=/snapshots/)(.*?)\.jpg", response.text)))
        )
        ...
        if response.status_code == 200:
            ...
            if "Last-Modified" in dict_.keys():
                dt = datetime.strptime(
                    dict_["Last-Modified"], "%a, %d %b %Y %H:%M:%S %Z"
                )
                dt_ = dt.replace(tzinfo=pytz.utc).astimezone(
...
if __name__ == "__main__":
    ...
    for yyyy_mm_dd in df.LMD.drop_duplicates().values:
        parquet_key = (
            f"mark/cctvlogs/date={yyyy_mm_dd}/cctvlog_{yyyy_mm_dd}_{hmsf}.parquet"
        )
