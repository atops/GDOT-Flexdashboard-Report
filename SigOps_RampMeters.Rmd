...
timeseries_plot <- function(df, x_variable, y_variable, color_variable) {
    plot_ly() %>%
        add_trace(
            data = df,
            type = "scatter", 
            ...
            line = list(
                color = "lightgray")) %>%
        add_trace(
            data = filter(df, is.na(`Metering Rate`)),
            name = "",
            type = "scatter", 
            mode = "markers", 
            x = ~Timestamp, 
            ...
            customdata = ~glue(paste(
                ...
                "<br><b>Avg Flow Rate:</b> {`Avg Flow Rate`}",
                "<br><b>Metering Rate:</b> {`Metering Rate`}")),
...
observe({write_parquet(events(), "events.parquet")})
...
observe({write_parquet(traffic(), "traffic.parquet")})
...
observe({write_parquet(df_scatter2(), "df_scatter2.parquet")})
