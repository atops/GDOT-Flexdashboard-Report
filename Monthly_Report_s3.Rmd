...
source("Monthly_Report_UI_Functions.R", local = TRUE)
...
corridors <- qs::qread("all_Corridors_Latest.qs")
cor <- qs::qread("cor.qs")
sig <- qs::qread("sig.qs")
sub <- qs::qread("sub.qs")
...
output$corridor_summary_table <- renderFormattable({
            shiny::validate(need(corridor_summary_table(),
                          "Select a Zone for the Metrics Summary Table"))
            corridor_summary_table()
        })

fillRow(
    formattableOutput("corridor_summary_table")
)
fillRow(
    renderUI({
...
# This is the Production Version. Fixed reports. Just show html.

# When deploying production site, save down the latest. Uncomment the below:
# aws.s3::save_object(
#     object = "Zone_Manager_Report_Content.rds",
#     bucket = "gdot-spm",
#     file = "~/Code/GDOT/GDOT-Flexdashboard-Report/PRODUCTION/Zone_Manager_Report_Content.rds")

zmdf <- readRDS("Zone_Manager_Report_Content.rds") %>%
            group_by(Month, Zone) %>%
            top_n(10, LastModified) %>%
            ungroup()
