...
group_corridors_ <- function(df, per_, var_, wt_, gr_ = group_corridor_by_) {
    ...
    df_ <- df %>%
        split(df$Zone_Group)
    all_zones <- lapply(names(df_), function(x) {
        gr_(df_[[x]], per_, var_, wt_, x)
    })
    ...
    all_zone_groups <- lapply(names(df_), function(x) {
        gr_(df_[[x]], per_, var_, wt_, x)
    })

    # concatenate all summaries with corridor averages
    dplyr::bind_rows(
        select(df, "Corridor", Zone_Group = "Zone", !!per_, !!var_, !!wt_, "delta"),
        all_zone_groups
    ) %>%
        distinct() %>%
        mutate(
...
get_weekly_avg_by_day_cctv <- function(df, var_ = "uptime", wt_ = "num") {
    ...
    df %>%
        ...
        left_join(Tuesdays) %>%
        dplyr::select(
            Zone_Group, Zone, Corridor, Subcorridor, CameraID,
...
get_avg_by_hr <- function(df, var_, wt_ = NULL) {
    ...
    if (is.null(wt_)) {
        ...
        ret <- df_[, .(
            weighted.mean(get(var_), get(wt_), na.rm = TRUE),
...
summarize_by_peak <- function(df, date_col) {
    ...
    df %>%
        mutate(
            Peak = if_else(hour(!!date_col_) %in% AM_PEAK_HOURS | hour(!!date_col_) %in% PM_PEAK_HOURS,
                "Peak",
                "Off_Peak"
            ),
            Peak = factor(Peak)
        ...
        lapply(function(x) {
            x %>%
                ...
                summarize(
                    sf_freq = weighted.mean(sf_freq, cycles),
                    cycles = sum(cycles),
                    .groups = "drop"
                ) %>%
                select(-Peak)
...
get_daily_detector_uptime <- function(filtered_counts) {
    ...
    if (nrow(ddu) > 0) {
        ddu <- ddu %>%
            mutate(
                setback = ifelse(CallPhase %in% c(2, 6), "Setback", "Presence"),
            ...
            lapply(function(x) {
                x %>%
                    ...
                    summarize(
                        uptime = sum(Good_Day, na.rm = TRUE),
                        all = n(),
                        .groups = "drop"
                    ) %>%
                    mutate(
...
get_monthly_vph <- function(vph) {
    vph %>%
        ...
        group_by(SignalID, Hour) %>%
        summarize(
            vph = sum(vph, na.rm = TRUE),
            .groups = "drop_last"
        ) %>%
        mutate(Hour = Hour - days(day(Hour)) + days(1)) %>%
        group_by(SignalID, Hour) %>%
        summarize(
            vph = mean(vph, na.rm = TRUE),
            .groups = "drop"
        )
}
...
get_monthly_aog_by_hr <- function(aog_by_hr) {
    aog_by_hr %>%
        group_by(SignalID, Hour) %>%
        summarize(
            aog = weighted.mean(aog, vol, na.rm = TRUE),
            vol = sum(vol, na.rm = TRUE),
            .groups = "drop_last"
        ) %>%
        mutate(Hour = Hour - days(day(Hour)) + days(1)) %>%
...
get_cor_weekly_ti <- function(ti, cor_weekly_vph, corridors) {
    ...
    cor_weekly_vph <- cor_weekly_vph %>%
        mutate(Week = week(as_date(Hour))) %>%
        left_join(tuesdays, by = "Week")
