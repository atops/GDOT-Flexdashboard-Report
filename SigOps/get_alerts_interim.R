...
# Upload to S3 Bucket: mark/interim/watchdog/
tryCatch({
    s3write_using(
        alerts,
        qsave,
        bucket = conf$bucket,
        object = "mark/interim/watchdog/alerts.qs",
    ...
    s3write_using(
        ...
        write_parquet,
        bucket = conf$bucket,
        object = "mark/interim/watchdog/alerts.parquet",
        opts = list(multipart = TRUE))

    write(
        glue(paste0(
            "{format(now(), '%F %H:%M:%S')}|SUCCESS|get_alerts.R|get_alerts|Line 173|",
            "Uploaded {conf$bucket}/mark/interim/watchdog/alerts.qs")),
