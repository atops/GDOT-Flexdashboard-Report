...
as_int <- function(x) {
    scales::comma_format()(as.integer(x))
}
as_2dec <- function(x) {
    sprintf(x, fmt = "%.2f")
}
...
read_signal_data <- function(conn, signalid, plot_start_date, plot_end_date) {
    ...
    q <- glue(paste(
        "select signalid, date, dat from signal_details, unnest (data) t(dat)",
...
get_valuebox <- function(cor_monthly_df, var_, var_fmt, break_ = FALSE,
    ...
    if (is.null(qu)) { # want monthly, not quarterly data
        vals <- cor_monthly_df %>%
            dplyr::filter(Corridor == zone & Month == mo) %>%
            as.list()
    } else {
        vals <- cor_monthly_df %>%
            dplyr::filter(Corridor == zone & Quarter == qu) %>%
            as.list()
    }
...
get_bar_line_dashboard_plot <- function(cor_weekly,
    ...
    if (nrow(mdf) > 0 & nrow(wdf) > 0) {
        ...
        weekly_line_chart <- plot_ly(sdw,
            ...
            layout(
                xaxis = list(title = x_line1_title),
                yaxis = list(
                    tickformat = tickformat_,
        ...
        if (!is.null(cor_hourly)) {
            ...
            hourly_line_chart <- plot_ly(sdh) %>%
                ...
                layout(
                    xaxis = list(title = x_line2_title),
...
get_tt_plot <- function(cor_monthly_tti, cor_monthly_tti_by_hr,
    ...
    if (nrow(mott) > 0 & nrow(hrtt) > 0) {
        ...
        pttimo <- base_m %>%
            ...
            layout(
                xaxis = list(title = "Travel Time Index (TTI"),
                yaxis = list(
                    range = c(1, mo_max),
                    tickformat = tickformat
                ),
        ...
        pptimo <- base_m %>%
            ...
            layout(
                xaxis = list(title = "Planning Time Index (PTI)"),
                yaxis = list(
                    range = c(1, mo_max),
                    tickformat = tickformat
                ),
    ...
    plot_detector_uptime_bar <- function(df) {
        ...
        plot_ly(
            ...
            layout(
                barmode = "overlay",
                xaxis = list(
                    title = paste(month_name, "Detector Uptime (%)"),
                    zeroline = FALSE,
                    tickformat = "%"
                ),
                yaxis = list(title = ""),
    ...
    plot_detector_uptime_bar <- function(df) {
        ...
        plot_ly(
            ...
            layout(
                barmode = "overlay",
                xaxis = list(
                    title = paste(month_name, "Comms Uptime (%)"),
                    zeroline = FALSE,
                    tickformat = "%"
                ),
                yaxis = list(title = ""),
...
cum_events_plot <- function(df) {
    plot_ly(height = 300) %>%
        add_bars(
            data = filter(df, Events == "Reported"),
            x = ~Month,
            y = ~Status,
            name = "Reported",
        ...
        add_bars(
            data = filter(df, Events == "Resolved"),
            x = ~Month,
            y = ~Status,
            name = "Resolved",
        ...
        add_trace(
            data = filter(df, Events == "Outstanding"),
            x = ~Month,
            y = ~Status,
    ...
    if (nrow(df) > 0) {
        ...
        p <- ggplot() +
            ...
            labs(
                x = "",
                y = ""
            ) + # Intersection (and phase, if applicable)") +


            # draw white gridlines between tick labels
            geom_vline(
                xintercept = as.numeric(seq(start_date, end_date, by = "1 day")) - 0.5,
                color = "white"
            )
        ...
        if (length(unique(df$signal_phase)) > 1) {
            p <- p +
                geom_hline(
                    yintercept = seq(1.5, length(unique(df$signal_phase)) - 0.5, by = 1),
                    color = "white"
                )
        }
        p
