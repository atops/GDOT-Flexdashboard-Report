...
shinyApp(
    ui = fluidPage(
        ...
        #                           selected = 1,
        #                           width = "150px")),
        div(absolutePanel(top = 10, left = 100,  # 260,
                      selectInput(inputId = "metric_input", 
                                  label = "Select Metric", 
        ...
        #                           selected = 1,
        #                           width = "150px")),
        div(absolutePanel(top = 10, left = 430,
                      radioButtons("filter_input", label = "Filter by",
                      choices = list(
                          "View all signals" = 1, 
                          "Filter by signal group in sidebar" = 2), 
                      selected = 1)),
            style = "font-family: Source Sans Pro"),
        div(absolutePanel(top = 110, left = 50,
                          dataTableOutput("top10_table")),
            style = "font-family: Source Sans Pro; font-size: 14px; background-color: rbga(0,0,0,0.4)")
    ),
    ...
    server = function(input, output, session) {
        ...
        data <- reactive({
            ...
            cat(file = stderr(), "table =", table(), ", class=", class(table()), "\n")
            cat(file = stderr(), "value =", value(), ", class=", class(value()), "\n")
...
observe({
    ...
    maintenance_cols_to_show <- c(
        maintenance_cols$default, maintenance_cols[input$maintenance_cols_checked]
        ) %>% 
        unlist() %>% unname()
...
observe({
    ...
    operations_cols_to_show <- c(
        operations_cols$default, operations_cols[input$operations_cols_checked]
        ) %>% 
        unlist() %>% unname()
    ...
    print(operations_cols_to_show)
    print(operations_hide_cols)
...
output$operations_table <- renderDT({
    ...
    get_monthly_operations_health_table(
        df, current_month(), input$zone_group, input$corridor_x)
})
...
selectInput("operations_var", 
            ...
            #choices = "Percent Health",
            #choices = maintenance_metric_choices(),
...
#         
#         if (corridor() != "All Corridors") {
#             df <- filter(df, !(Zone == Corridor))  %>%
#                 mutate(
#                     Zone_Group = Corridor,
#                     Corridor = Subcorridor,
#                     Description = Subcorridor
#                 )
#             operations_var <- input$operations_var_
#         } else {
#             df <- filter(df, Corridor == Subcorridor | Zone == Corridor) %>%
#                 rename(Zone_Group = Zone) %>%
#                 mutate(Description = Corridor)
#             operations_var <- "Percent Health"
#         }
#         
#         if (grepl("Score", operations_var) | operations_var == "Ped Delay") {
#             num_format_ = "integer"
#         } else if (operations_var %in% c("Platoon Ratio", "Travel Time Index", "Buffer Index")) {
#             num_format_ = "decimal"
#         } else
#             num_format_ = "percent"
#         
#         get_bar_line_dashboard_plot(
#             cor_weekly = rename(df, Date = Month),
#             cor_monthly = df,
#             cor_hourly = NULL,
#             var_ = operations_var,
#             num_format = num_format_,
#             highlight_color = GDOT_BLUE,
#             month_ = current_month(),
#             zone_group_ = zone_group(),
#             x_bar_title = paste(input$month, operations_var),
#             x_line1_title = paste0(operations_var," Trend"),
#             plot_title = operations_var
#         )
#     }), height = "700px"
# )
