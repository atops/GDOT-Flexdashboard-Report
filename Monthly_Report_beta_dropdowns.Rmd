...
shinyApp(
    ...
    server = function(input, output, session) {
        ...
        table_data <- reactive({
            debug(logger, "table_data")

            v <- as.name(map_metric()$variable)
            
            data_ <- if (map_metric()$variable == "uptime") {
                map_data() %>% arrange(!!v)  # map_metric()$variable)
            } else {
                map_data() %>% arrange(desc(!!v))  # map_metric()$variable))
            }
        ...
        output$top10_table <- renderDataTable({
            debug(logger, "output$top10_table")
            datatable(
        ...
        observe({
            debug(logger, "observe on data(): update map")
        ...
        observeEvent(input$top10_table_rows_selected, {
            debug(logger, "observe event on top10 table row selected")
...
```{r Maintenance, warning = FALSE, eval = FALSE}
...
```{r Operations, warning = FALSE, eval = FALSE}
