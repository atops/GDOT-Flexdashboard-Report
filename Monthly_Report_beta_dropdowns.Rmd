...
shinyApp(
    ...
    server = function(input, output, session) {
            ...
            if (nrow(data())) {
                ...
                leafletProxy("MapPlot1") %>%
                    addCircleMarkers(
                        ...
                        popup = paste(
                            sep = "<br/>",
                            glue("Signal {data()$Description}"),
                            glue(paste(
                                "<b>{input$metric_input}:</b>", 
                                "{data_format(map_metric()$data_type)(data()[[map_metric()$variable]])}"))
                        ),
                        radius = 5,
                        fillColor = pal(data()[[map_metric()$table]]),
                        stroke = FALSE,
                        fillOpacity = 1,
                        group = "the_group"
                    ) %>%
        ...
        data <- reactive({
            query_data(
                map_metric(), 
                level = "signal", resolution = "monthly", zone_group = zone_group(), month = current_month(), upto = FALSE
            ) %>%
            ...
            #data$Description <- iconv(data$Description, "latin1", "UTF-8")  # From before. Not sure if still needed.
            #data
        })
        ...
        table_data <- reactive({
            ...
            data_ <- if (map_metric()$variable == "uptime") {
                data() %>% arrange(map_metric()$variable)
            } else {
                data() %>% arrange(desc(map_metric()$variable))
            }
