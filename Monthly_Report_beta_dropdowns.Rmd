...
tp_valuebox <- reactive({
    valueBox(
        value = get_valuebox_value(
            throughput,
            level = level(), zone_group = zone_group(), month = current_month(), line_break = TRUE
        ),
        icon = "fa-bar-chart",
        color = BLUE
    )
...
vpd_valuebox <- reactive({
    valueBox(
        value = get_valuebox_value(
            vpd,
            level = level(), zone_group = zone_group(), month = current_month(), line_break = FALSE
        ),
        icon = "fa-area-chart",
        color = BLUE
    )
...
renderValueBox({
    valueBox(
        value = get_valuebox_value(
            throughput,
            level = level(), zone_group = zone_group(), quarter = current_quarter(), line_break = TRUE
        ),
        icon = "fa-bar-chart",
        color = GDOT_BLUE
    )
...
renderValueBox({
    valueBox(
        value = get_valuebox_value(
            vpd,
            level = level(), zone_group = zone_group(), quarter = current_quarter(), line_break = FALSE
        ),
        icon = "fa-area-chart",
...
output$performance <- renderPlotly({
    if (input$performance_metric == "Throughput") {
        get_trend_multiplot(
            throughput, 
            level = level(), zone_group(), current_month()
        )
...
output$equipment <- renderPlotly({
    ...
    if (input$equipment_metric == "Detector Uptime") {
        ...
        if (corridor() == "All Corridors" || input$subcorridors == "By Sub-corridor") {
            debug(logger, glue("Line 1681 | {zone_group()} | {current_month()}"))
            get_trend_multiplot(
                cctv_uptime, 
                level = level(), zone_group(), current_month()
            )
        } else {
            debug(logger, glue("Line 1687 | {zone_group()} | {current_month()}"))
            individual_cctvs_plot(zone_group(), current_month())
        }
...
fillRow(
    fillCol(
        renderPlotly({
            data <- query_data(
                tasks, 
                level = level(), "monthly", zone_group = zone_group(), month = current_month()
                )
        ...
        renderPlotly({
            data <- query_data(
                tasks_by_type,
                level = level(), "monthly", zone_group = zone_group(), month = current_month(), upto = FALSE
                )
            shiny::validate(need(nrow(data) > 0, "No Data"))
            data <- data[data$Corridor == zone_group(),]
...
output$udc_plot <- renderPlotly({
    udcplot_plotly(
        query_udc_hourly(zone_group(), current_month())
    )
})
