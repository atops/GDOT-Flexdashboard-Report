...
### Pedestrian Pushbotton Availability {.value-box}
...
checkboxGroupInput(
    ...
    selected = NULL,  # c("scores", "metrics"),
    inline = TRUE
)

maintenance_cols <- list(
    "all" = c(0:18),
    "default" = c(0:6), 
    "scores" = c(7:12), 
    "metrics" = c(13:18))
...
output$maintenance_table <- renderDT({
    ...
    get_monthly_maintenance_health_table(
        df, current_month(), zone_group(), corridor()) 
})
...
output$operations_table <- renderDT({
    if( corridor() == "All Corridors" | input$subcorridors == "By Sub-corridor" ){
        df <- subdata()$mo$ops # operations_sub
    } else {
        df <- sigdata()$mo$ops # operations_sig
    }

    get_monthly_operations_health_table(
        df, current_month(), zone_group(), corridor())
})
...
observe({
    dataTableProxy("operations_table") %>% 
        hideCols(operations_hide_cols, reset = TRUE)
})
...
selectInput("operations_var", 
            "Operations Health Metric:",
            #choices = names(operations_sub)[7:17],
            choices = c("Percent Health", "Missing Data", "Platoon Ratio Score", 
